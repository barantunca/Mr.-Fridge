"""
widgets.py — Reusable, styled Kivy components
"""
...
class CardWidget(BoxLayout):
    """Card with rounded corners and a background."""
...
class StyledLabel(Label):
    """Label with default color and text settings."""
...
class GradientButton(Button):
    """Button with a purple gradient background."""
    ...
    def _redraw(self, *args):
        ...
        self.canvas.before.clear()
        with self.canvas.before:
            # Gradient simulation (left → right blend of two colors)
            Color(*ACCENT)
...
class CustomTopBar(BoxLayout):
    """Custom top bar displayed at the top of each screen."""
...
class ProductCard(CardWidget):
    """Product/ingredient card used in the Quick List and Inventory screens."""
    def __init__(self, name, days_left, total_days=14, **kwargs):
        ...
        # Image (using mascot as placeholder)
        ...
        days_str = "EXPIRED" if days_left <= 0 else f"{days_left} DAYS LEFT"
